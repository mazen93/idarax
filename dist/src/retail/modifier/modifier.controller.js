"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ModifierController = void 0;
const common_1 = require("@nestjs/common");
const modifier_service_1 = require("./modifier.service");
const jwt_auth_guard_1 = require("../../auth/jwt-auth.guard");
const permissions_decorator_1 = require("../../auth/permissions.decorator");
const permissions_constants_1 = require("../../auth/permissions.constants");
const swagger_1 = require("@nestjs/swagger");
let ModifierController = class ModifierController {
    modifierService;
    constructor(modifierService) {
        this.modifierService = modifierService;
    }
    getAll(productId) {
        return this.modifierService.getForProduct(productId);
    }
    createGroup(productId, dto) {
        return this.modifierService.createGroup(productId, dto);
    }
    updateGroup(groupId, dto) {
        return this.modifierService.updateGroup(groupId, dto);
    }
    deleteGroup(groupId) {
        return this.modifierService.deleteGroup(groupId);
    }
    addOption(groupId, dto) {
        return this.modifierService.addOption(groupId, dto);
    }
    updateOption(optionId, dto) {
        return this.modifierService.updateOption(optionId, dto);
    }
    deleteOption(optionId) {
        return this.modifierService.deleteOption(optionId);
    }
};
exports.ModifierController = ModifierController;
__decorate([
    (0, common_1.Get)(),
    (0, permissions_decorator_1.Permissions)(permissions_constants_1.Actions.CATALOG.VIEW),
    (0, swagger_1.ApiOperation)({ summary: 'Get all modifier groups + options for a product' }),
    __param(0, (0, common_1.Param)('productId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], ModifierController.prototype, "getAll", null);
__decorate([
    (0, common_1.Post)('groups'),
    (0, permissions_decorator_1.Permissions)(permissions_constants_1.Actions.CATALOG.EDIT),
    (0, swagger_1.ApiOperation)({ summary: 'Create a modifier group (e.g. "Sauce", "Size")' }),
    __param(0, (0, common_1.Param)('productId')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", void 0)
], ModifierController.prototype, "createGroup", null);
__decorate([
    (0, common_1.Patch)('groups/:groupId'),
    (0, permissions_decorator_1.Permissions)(permissions_constants_1.Actions.CATALOG.EDIT),
    (0, swagger_1.ApiOperation)({ summary: 'Update a modifier group' }),
    __param(0, (0, common_1.Param)('groupId')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", void 0)
], ModifierController.prototype, "updateGroup", null);
__decorate([
    (0, common_1.Delete)('groups/:groupId'),
    (0, permissions_decorator_1.Permissions)(permissions_constants_1.Actions.CATALOG.DELETE),
    (0, swagger_1.ApiOperation)({ summary: 'Delete a modifier group and all its options' }),
    __param(0, (0, common_1.Param)('groupId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], ModifierController.prototype, "deleteGroup", null);
__decorate([
    (0, common_1.Post)('groups/:groupId/options'),
    (0, permissions_decorator_1.Permissions)(permissions_constants_1.Actions.CATALOG.EDIT),
    (0, swagger_1.ApiOperation)({ summary: 'Add an option to a modifier group' }),
    __param(0, (0, common_1.Param)('groupId')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", void 0)
], ModifierController.prototype, "addOption", null);
__decorate([
    (0, common_1.Patch)('groups/:groupId/options/:optionId'),
    (0, permissions_decorator_1.Permissions)(permissions_constants_1.Actions.CATALOG.EDIT),
    (0, swagger_1.ApiOperation)({ summary: 'Update an option' }),
    __param(0, (0, common_1.Param)('optionId')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", void 0)
], ModifierController.prototype, "updateOption", null);
__decorate([
    (0, common_1.Delete)('groups/:groupId/options/:optionId'),
    (0, permissions_decorator_1.Permissions)(permissions_constants_1.Actions.CATALOG.DELETE),
    (0, swagger_1.ApiOperation)({ summary: 'Delete an option' }),
    __param(0, (0, common_1.Param)('optionId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], ModifierController.prototype, "deleteOption", null);
exports.ModifierController = ModifierController = __decorate([
    (0, swagger_1.ApiTags)('Product Modifiers'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.Controller)('retail/products/:productId/modifiers'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __metadata("design:paramtypes", [modifier_service_1.ModifierService])
], ModifierController);
//# sourceMappingURL=modifier.controller.js.map