...
import { X, Upload, FileSpreadsheet, Download, AlertCircle, CheckCircle2, Loader2, RefreshCw } from 'lucide-react';
...
export const ImportCustomersModal: React.FC<ImportCustomersModalProps> = ({ isOpen, onClose, onSuccess, t }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            const res = await api.post(`/import/customers?mode=${mode}`, formData, {
                headers: {
    ...
    const downloadTemplate = () => {
        const headers = ['Name', 'Phone', 'Email', 'Points', 'AddressLabel', 'Address', 'IsDefault'].join(',');
        const example1 = 'John Doe,+1234567890,john@example.com,100,Home,"123 Main St, New York",true';
        const example2 = 'John Doe,+1234567890,,,,Work,"456 Office Rd, New York",false';
        const example3 = 'Jane Smith,+0987654321,jane@smith.com,50,Home,"789 Garden Ave, London",true';

        const csvContent = "data:text/csv;charset=utf-8," + [headers, example1, example2, example3].join('\n');
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-[#1a1c23] border border-border rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-border flex justify-between items-center bg-gradient-to-r from-primary-500/10 to-transparent">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-primary-500/20 rounded-lg">
                            <FileSpreadsheet className="w-5 h-5 text-primary-400" />
                        </div>
                        <div>
                            <h3 className="text-lg font-bold text-white">{t('import_customers') || 'Import Customers'}</h3>
                            <p className="text-white/40 text-xs">Bulk upload customers via CSV or Excel</p>
                        </div>
                    </div>
                ...
                <div className="p-8 space-y-6 overflow-y-auto">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                ...
                                className={`flex flex-col items-center gap-2 p-4 rounded-xl border transition-all ${mode === 'SKIP_EXISTING' ? 'bg-primary-500/10 border-primary-500/50 text-primary-400 ring-2 ring-primary-500/20' : 'bg-white/5 border-border text-white/40 hover:bg-white/10'}`}
                            >
                                <CheckCircle2 className={`w-5 h-5 ${mode === 'SKIP_EXISTING' ? 'text-primary-400' : 'text-white/20'}`} />
                                <span className="text-sm font-bold">Skip Existing</span>
                                <span className="text-[10px] opacity-60">Match by Phone</span>
                            </button>
                            <button
                            ...
                            >
                                ...
                                <span className="text-[10px] opacity-60">Update by Phone</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <label className={`relative group flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-2xl cursor-pointer transition-all ${file ? 'border-primary-500/50 bg-primary-500/5' : 'border-border hover:border-white/20 bg-white/[0.02] hover:bg-white/[0.04]'}`}>
                            <input type="file" accept=".csv, .xlsx, .xls" onChange={handleFileChange} className="hidden" />
                            <div className={`p-4 rounded-2xl mb-4 transition-transform group-hover:scale-110 ${file ? 'bg-primary-500/20' : 'bg-white/5'}`}>
                                <Upload className={`w-8 h-8 ${file ? 'text-primary-400' : 'text-white/20'}`} />
                            </div>
                            ...
                            <div className="p-4 bg-primary-500/10 border border-primary-500/20 rounded-xl space-y-2">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="bg-primary-500/10 rounded-lg p-2 text-center">
                                        <div className="text-lg font-bold text-primary-400">{result.imported}</div>
                                        <div className="text-[10px] text-primary-400/60 uppercase">New</div>
                                    </div>
                ...
                <div className="p-6 border-t border-border bg-white/[0.01] flex gap-3">
                    ...
                    <button
                        ...
                        className="flex-[2] py-3 bg-primary-600 hover:bg-primary-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-xl shadow-lg shadow-primary-500/20 transition-all flex items-center justify-center gap-2"
                    >
